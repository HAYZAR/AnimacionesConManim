...
2. Crea 3 hojas con estos nombres **exactos**: `Banco`, `Resultados`,
   `Conversaciones`.
...
6. Autoriza los permisos solicitados (Sheets, llamadas externas a
...
4. Pulsa "Ir a prueba adaptativa" y confirma que aparecen las tarjetas de
   pregunta dentro del propio chat (no debería abrir ninguna pestaña nueva).
5. Responde el test y envíalo; confirma que aparece la pantalla de
   resultados y que se agregaron filas nuevas en `Resultados`.
...
- **El frontend recibe un error de red / respuesta vacía**: confirma que la
  implementación web tiene acceso "Cualquier usuario" (si la Sheet es de una
  cuenta de colegio con políticas restrictivas, puede requerir aprobación
  del administrador de Workspace).
...
- **Las respuestas del test no aparecen en `Resultados`**: revisa el
  Registro de ejecución de Apps Script al enviar el test — `calificarTest`
  lanza error si la hoja `Resultados` no existe con ese nombre exacto.
